"""066: NCHW average pooling excluding padded samples."""
...
def op066_avg_pool2d(
    x: torch.Tensor,
    kernel: int | tuple[int, int] = (2, 2),
    stride: int = 2,
    padding: int = 0,
) -> torch.Tensor:
    """Average-pool NCHW with ``count_include_pad=False`` semantics."""
    ...
    if out.numel():
        ...
        _op066_avg_pool2d_kernel[(out.numel(),)](
            x,
            out,
            HEIGHT=height,
            WIDTH=width,
