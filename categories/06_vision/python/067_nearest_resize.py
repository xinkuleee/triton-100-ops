"""067: floor-mapped nearest-neighbor NCHW resize."""
...
@triton.jit
def _op067_nearest_resize_kernel(
    x_ptr,
    out_ptr,
    total,
...
def op067_nearest_resize(
    x: torch.Tensor, out_height: int, out_width: int
) -> torch.Tensor:
    """Resize contiguous NCHW using floor-mapped nearest neighbors."""
    ...
    if out.numel():
        _op067_nearest_resize_kernel[(triton.cdiv(out.numel(), RESIZE_BLOCK),)](
            x,
            out,
            out.numel(),
            CHANNELS=channels,
