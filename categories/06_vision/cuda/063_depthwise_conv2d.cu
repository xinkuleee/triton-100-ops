...
namespace gpu_ops {
namespace {
__global__ void op063_depthwise_conv2d_kernel(
    const float* x, const float* weight, const float* bias, float* out,
    int total, int channels, int height, int width, int kernel_height,
    int kernel_width, int out_height, int out_width, int stride, int padding) {
  ...
  const int batch = output_index / (out_width * out_height * channels);
      ...
      if (input_y >= 0 && input_y < height && input_x >= 0 &&
        ...
        accumulator +=
            x[((batch * channels + channel) * height + input_y) * width +
              input_x] *
            weight[(channel * kernel_height + kernel_y) * kernel_width +
                   kernel_x];
    }
...
cudaError_t launch_op063_depthwise_conv2d(
    const float* x, const float* weight, const float* bias, float* out,
    int batch, int channels, int height, int width, int kernel_height,
    int kernel_width, int stride, int padding, cudaStream_t stream) {
  ...
  op063_depthwise_conv2d_kernel<<<
      ceil_div_int(total, kThreads), kThreads, 0, stream>>>(
      x, weight, bias, out, total, channels, height, width, kernel_height,
      kernel_width, out_height, out_width, stride, padding);
