...
@triton.jit
def _op096_adam_kernel(
    param_ptr, grad_ptr, first_ptr, second_ptr, learning_rate, beta1, beta2,
    eps, correction1, correction2, count, BLOCK: tl.constexpr,
):
...
def op096_adam_(
    ...
    if (
        ...
        raise ValueError("step >= 1, beta values in [0,1), and eps > 0 are required")
    correction1, correction2 = 1.0 - beta1 ** step, 1.0 - beta2 ** step
    count = param.numel()
    if count:
        _op096_adam_kernel[(triton.cdiv(count, 256),)](
            param, grad, first_moment, second_moment, learning_rate, beta1, beta2,
            eps, correction1, correction2, count, BLOCK=256, num_warps=4,
        )
    return param
