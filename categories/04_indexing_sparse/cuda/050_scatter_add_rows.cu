...
// Atomics serialize colliding updates but guarantee none are lost.
__global__ void op050_scatter_add_rows_kernel(
...
inline bool op050_product_fits_int(int left, int right) {
  return left == 0 || right <= INT_MAX / left;
}

}  // namespace

cudaError_t launch_op050_scatter_add_rows(
    const float* src, const int64_t* indices, float* out,
    int rows, int cols, int count, cudaStream_t stream) {
  if (rows < 0 || cols <= 0 || count < 0 ||
      !op050_product_fits_int(rows, count)) return cudaErrorInvalidValue;
  const int total = rows * count;
  if (total == 0) return cudaSuccess;
  op050_scatter_add_rows_kernel
